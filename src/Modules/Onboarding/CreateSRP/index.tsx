...
const CSRP = () => {
  ...
  const text =
    "spring glow try total hurry odor doctor text boil coil wrap address";
  const arr = text.split(" ");
  const phraseJSX = useMemo(() => {
    return arr.map((word, index) => (
      <span key={index} className="text-neutral-400  text-center  w-[150px]  p-2">
  ...
  return (
    <div className="">
      ...
      <div className="flex items-center justify-center flex-col">
        ...
        <div className="flex items-center justify-center">
        <span className="text-neutral-400 ">{copy?'Copied!':'Click anywhere on this card to copy'}</span>
        </div>
        </div>
